...
public class BackGroundBlueScript : MonoBehaviour{
    ...
    void Start()
    {
        ColorValue = PlayerPrefs.GetInt("BackGroundBlue", 235);
        TextBox.text = ColorValue.ToString();
    }

    public void BackGroundBlueButton()
    {
        PlayerPrefs.SetInt("BackGroundColorValue", 3);
    ...
    void Update()
    {
        ...
        if (IntValue == 3)
        {
            if (!keys.active)
            {
                ...
                PlayerPrefs.SetInt("BackGroundBlue", ColorValue);
